...
The temperature, humidity and luminance sensors are installed inside the node, in order to evaluate internal prototype conditions. It is expected that internal temperature and humidity increases proportionally to CPU utilization, for example.

The luminance sensor actually measures light conditions inside the tank, since node prototype has a transparent case. Since VL53L1X performance may vary with light interference, an "open hatch" alarm should be created, in case light is detected inside the water tank. Open water tanks are a very bad idea, due to mosquito proliferation!
